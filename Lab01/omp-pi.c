...
{
    size_t npoints = 10000, ninside=0;
    double pi_approx;
    ...
    size_t inside[omp_get_num_threads()];
    ...
    {
        size_t size = npoints / omp_get_num_threads();
        ninside += generate_points(size);
    }
    t_end = omp_get_wtime();
