 ...
 * Copyright (C) 2016, 2017 Moreno Marzolla <moreno.marzolla@unibo.it>
 *
 ...
 * ---------------------------------------------------------------------------
 * Usage:
 *   compile:  gcc -std=c99 -Wall -Wpedantic -fopenmp -O2 -o c-ray c-ray.c -lm
 *   run:      ./c-ray -s 1280x1024 < sphfract.small.in > foo.ppm
...
void render(int xsz, int ysz, pixel_t *fb, int samples)
{
     ...
     */
    #pragma omp parallel for private(j,i,s) collapse(2)
    for (j=0; j<ysz; j++) {
        for (i=0; i<xsz; i++) {
            double r, g, b;
            r = g = b = 0.0;
