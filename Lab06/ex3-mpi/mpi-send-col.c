/****************************************************************************
 *
 * mpi-send-col.c - Exercise for exchanging ghost columns with MPI
 *
...
/* Initialize matrix m with the values k, k+1, k+2, ..., from left to
   ...
   (size+2)*size integers. The first and last column of m are the ghost cells,
   that are set to -1. */
...
int main( int argc, char *argv[] )
{
    ...
    if ( 0 == my_rank && 2 != comm_sz ) {
        printf("You must execute exactly 2 processes\n");
